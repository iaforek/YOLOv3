...
def split_heads_to_grid(pred_flat, num_classes=80, A=3):
...
def infer_one(img_path, model, num_classes=80, conf_thres=0.25, iou_thres=0.5, out_path="pred_vis.jpg"):
    ...
    print("max conf (obj*cls):", float(conf.max()))# (total, 5+C)
    ...
    with torch.no_grad():
        # preds_flat: (1, total, 5+C)
        obj = preds_flat[0, :, 0].sigmoid()
        topk = torch.topk(obj, k=min(5, obj.numel())).values.cpu().numpy()
        print(topk)

    # draw on 416×416 image
    vis = img.copy()
    if boxes[0].numel():
        for (x1,y1,x2,y2), s, c in zip(boxes[0].cpu().numpy(), scores[0].cpu().numpy(), classes[0].cpu().numpy()):
            x1,y1,x2,y2 = map(int, [x1,y1,x2,y2])
            cv2.rectangle(vis, (x1,y1), (x2,y2), (0,255,0), 2)
            cv2.putText(vis, f"{int(c)} {s:.2f}", (x1, max(0,y1-5)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,255,0), 1)
...
if __name__ == "__main__":
    ...
    # If you built anchors at runtime during training, re-run that code here or serialize them.
    # For a quick test you can use classic YOLOv3 COCO anchors (in pixels), then /stride when building the model.
    anchors_px = KMeansAnchors(collect_gt_boxes_np("coco128", 416, "train2017"), n_anchors=9).get_anchors()
    ...
    ckpt = "yolov3_scratch_coco128.pth"   # from your training
    model.load_state_dict(torch.load(ckpt, map_location="cpu"))
    model.to("cuda" if torch.cuda.is_available() else "cpu")

    # --- run on one image ---
    infer_one("coco128/images/train2017/000000000009.jpg", model, num_classes=NUM_CLASSES,

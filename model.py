...
class BoundingBoxPredictor(nn.Module):
    def __init__(self, anchors):
        super().__init__()
        A = len(anchors)
        self.register_buffer("anchors", anchors.view(1, 1, 1, A, 2).to(torch.float32))
        self.n_anchors_per_scale = A
    ...
    def forward(self, feature_map):
        ...
        # decode
        boxXY = torch.sigmoid(boxlocs[..., :2]) + idx                                   # (N,H,W,A,2)
        boxWH = torch.exp(boxlocs[..., 2:4].clamp(min=-4.0, max=4.0)) * self.anchors    # (N,H,W,A,2)

        bboxes = torch.cat((boxXY, boxWH), dim=-1)                        # (N,H,W,A,4)
...
class BoundingBoxPostProcessing:
    ...
    def non_max_suppression(self, scores: tuple, boxes: tuple, classes: tuple,
                            iou_threshold: float=0.5, class_agnostic: bool=False):
        outs = ([],[],[])
        for s, bxs, cls in zip(scores, boxes, classes):
            if s.numel()==0:
                outs[0].append(s); outs[1].append(bxs); outs[2].append(cls); continue
